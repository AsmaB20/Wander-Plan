...
function downloadPDF() {
  ...
  if (tr.startDate) doc.text(`${tr.startDate}  to  ${tr.endDate || '?'}`, margin + (tr.city ? 55 : 0), metaY);
  ...
  tr.days.forEach((d, di) => {
    ...
    if (d.places.length === 0) {
      ...
      d.places.forEach((p, pi) => {
        ...
        let meta = [];
        if (p.time) meta.push(`Time: ${p.time}`);
